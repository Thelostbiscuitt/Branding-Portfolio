...
var DEFAULT_FROM = "Habibcore <onboarding@resend.dev>";
var TO = "habib@habibcore.com";
...
__name(readLimitedJson, "readLimitedJson");
function validEnquiry(body) {
  if (!body || typeof body !== "object" || Array.isArray(body)) return false;
  const { name, email, projectType, timeline, brief } = body;
  const textWithin = /* @__PURE__ */ __name((value, max) => typeof value === "string" && value.length <= max, "textWithin");
  const optionalText = [[timeline, 120], [brief, 5e3]];
  const validTypes = projectType === void 0 || Array.isArray(projectType) && projectType.length <= 8 && projectType.every((item) => textWithin(item, 80));
  return textWithin(name, 120) && !!name.trim() && !/[\r\n\x00-\x1f]/.test(name) && textWithin(email, 254) && /^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}$/i.test(email) && optionalText.every(([value, max]) => value === void 0 || textWithin(value, max)) && validTypes;
}
__name(validEnquiry, "validEnquiry");
  ...
  return `
    <div style="font-family: monospace; max-width: 560px; color: #111;">
      <p style="font-size: 11px; letter-spacing: 0.1em; text-transform: uppercase; color: #888; margin-bottom: 24px;">
        New project enquiry \xB7 habibcore.com
      </p>
...
__name(notificationHtml, "notificationHtml");
function acknowledgementHtml({ safeName, safeProjectType, safeTimeline }) {
  return `
    <div style="font-family: monospace; max-width: 560px; color: #111;">
      <p style="font-size: 11px; letter-spacing: 0.1em; text-transform: uppercase; color: #888; margin-bottom: 24px;">
        Habibcore \xB7 habibcore.com
      </p>
      ...
      <p style="font-size: 11px; color: #999; margin-top: 32px;">Habib \xB7 Designer &amp; Builder \xB7 Lagos</p>
    </div>
  `;
}
__name(acknowledgementHtml, "acknowledgementHtml");
async function onRequestPost({ request, env }) {
  ...
  const safeProjectType = Array.isArray(projectType) ? projectType.map(esc).join(", ") : "Not specified";
  try {
...
function parseRange(header, size) {
  const match2 = /^bytes=(\d*)-(\d*)$/.exec(header);
  if (!match2 || !match2[1] && !match2[2] || size === 0) return null;
  const first = safeIndex(match2[1]);
  const last = safeIndex(match2[2]);
...
__name(audioResponse, "audioResponse");
async function onRequest({ request, env, params }) {
  ...
  if (request.method === "GET") {
    const options = range ? { range: { offset: range.start, length: range.end - range.start + 1 } } : void 0;
...
// ../.wrangler/tmp/pages-1YpIbe/functionsRoutes-0.11259537941186237.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-nbOpTl/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-nbOpTl/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.24197326027532995.mjs.map
